...
function a_base(word: any) {
  const sounds = [['う','わ'],['く','か'],['ぐ','が'],['す','さ'],['ず','ざ'],['つ','た'],['づ','だ'],['ぬ','な'],['ふ','は'],['ぶ','ば'],['ぷ','ぱ'],['む','ま'],['る','ら']];
  const ending = word.slug.substr(word.slug.length - 1);
  if (word.category === 'ichidan') {
    return '';
  } else {
    const sound = sounds.filter(function(arr: any){return arr[0] === ending;})[0];
    return sound[1];
  }
}
function i_base(word: any) {
  const sounds = [['う','い'],['く','き'],['ぐ','ぎ'],['す','し'],['ず','じ'],['つ','ち'],['づ','ぢ'],['ぬ','に'],['ふ','ひ'],['ぶ','び'],['ぷ','ぴ'],['む','み'],['る','り']];
  const ending = word.slug.substr(word.slug.length - 1);
  if (word.category === 'ichidan') {
    return '';
  } else {
...
function e_base(word: any) {
  const sounds = [['う','え'],['く','け'],['ぐ','げ'],['す','せ'],['ず','ぜ'],['つ','て'],['づ','で'],['ぬ','ね'],['ふ','へ'],['ぶ','べ'],['ぷ','ぺ'],['む','め'],['る','れ']];
  const ending = word.slug.substr(word.slug.length - 1);
  const sound = sounds.filter(function(arr: any){return arr[0] === ending;})[0];
  return sound[1];
}
function o_base(word: any) {
  const sounds = [['う','おう'],['く','こう'],['ぐ','ごう'],['す','そう'],['ず','ぞう'],['つ','とう'],['づ','どう'],['ぬ','のう'],['ふ','ほう'],['ぶ','ぼう'],['ぷ','ぽう'],['む','もう'],['る','ろう']];
  const ending = word.slug.substr(word.slug.length - 1);
  if (word.category === 'ichidan') {
    return 'よう';
  } else {
...
function te_base(word: any) {
  ...
  if (word.category === 'ichidan') {
    ...
    if (['問う','訪う','請う','乞う','恋う'].indexOf(word.slug) > -1) {
      return 'うて';
    } else if (['る','つ','う'].indexOf(ending) > -1 || word.slug === '行く') {
      return 'って';
...
const i_adjective = function(word: any) {
  const stem = {kanji: word.word.substring(0, word.word.length-1), kana: word.readings[0].substring(0, word.readings[0].length-1)};
  let gstem = {
    kanji: '',
    kana: ''
  };
  const last_sound = stem.kana[stem.kana.length-1];
  const kanji_stem_minus_one = (stem.kanji.substring(0,stem.kanji.length-1).length > 0) ? stem.kanji.substring(0,stem.kanji.length-1) : stem.kanji ;
  ...
  return {
    indicative: {
      ...
      polite: {
        positive: {kanji: word.word+'です', kana: word.readings[0]+'です'},
    ...
    presumptive: {
      plain: {
        positive: {kanji: word.word+'だろう', kana: word.readings[0]+'だろう'},
        negative: {kanji: stem.kanji+'くないだろう', kana: stem.kana+'くないだろう'}
      },
      polite: {
        positive: {kanji: word.word+'でしょう', kana: word.readings[0]+'でしょう'},
  ...
  return {
    indicative: {
      plain: {
        ...
        negative: {kanji: word.word+'ではない', kana: word.readings[0]+'ではない'}
      },
      polite: {
        positive: {kanji: word.word+'です', kana: word.readings[0]+'です'},
        negative: {kanji: word.word+'ではありません', kana: word.readings[0]+'ではありません'}
      }
    },
    past_indicative: {
      plain: {
        positive: {kanji: word.word+'だった', kana: word.readings[0]+'だった'},
        negative: {kanji: word.word+'ではなかった', kana: word.readings[0]+'ではなかった'}
      },
      polite: {
        positive: {kanji: word.word+'でした', kana: word.readings[0]+'でした'},
        negative: {kanji: word.word+'ではありませんでした', kana: word.readings[0]+'ではありませんでした'}
      }
    },
    presumptive: {
      plain: {
        positive: {kanji: word.word+'だろう', kana: word.readings[0]+'だろう'},
        negative: {kanji: word.word+'ではなかろう', kana: word.readings[0]+'ではなかろう'}
      },
      polite: {
        positive: {kanji: word.word+'でしょう', kana: word.readings[0]+'でしょう'},
        negative: {kanji: word.word+'ではないでしょう', kana: word.readings[0]+'ではないでしょう'}
      }
    },
    past_presumptive: {
      plain: {
        positive: {kanji: word.word+'だったろう', kana: word.readings[0]+'だったろう'},
        negative: {kanji: word.word+'ではなかったろう', kana: word.readings[0]+'ではなかったろう'}
      },
      polite: {
        positive: {kanji: word.word+'だったでしょう', kana: word.readings[0]+'だったでしょう'},
        negative: {kanji: word.word+'ではなかったでしょう', kana: word.readings[0]+'ではなかったでしょう'}
      }
    },
    provisional: {
      plain: {
        positive: {kanji: word.word+'なら', kana: word.readings[0]+'なら'},
        negative: {kanji: word.word+'でなければ', kana: word.readings[0]+'でなければ'}
      }
    },
    continuative: {
      plain: {
        positive: {kanji: word.word+'で', kana: word.readings[0]+'で'},
        negative: {kanji: word.word+'ではなくて', kana: word.readings[0]+'ではなくて'}
      },
      polite: {
        positive: {kanji: word.word+'でして', kana: word.readings[0]+'でして'},
        negative: {kanji: word.word+'ではありませんでして', kana: word.readings[0]+'ではありませんでして'}
      }
    },
    conditional: {
      plain: {
        positive: {kanji: word.word+'だったら', kana: word.readings[0]+'だったら'},
        negative: {kanji: word.word+'でなかったら', kana: word.readings[0]+'でなかったら'}
      },
      polite: {
        positive: {kanji: word.word+'でしたら', kana: word.readings[0]+'でしたら'},
        negative: {kanji: word.word+'でありませんでしたら', kana: word.readings[0]+'でありませんでしたら'}
      }
    },
    adverb: {kanji: word.word+'に', kana: word.readings[0]+'に'},
    become: {kanji: word.word+'になる', kana: word.readings[0]+'になる'},
    unbearably: {kanji: word.word+'でたまらない', kana: word.readings[0]+'でたまらない'},
